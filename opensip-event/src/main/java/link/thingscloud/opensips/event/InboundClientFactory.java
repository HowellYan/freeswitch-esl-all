...
import link.thingscloud.opensips.event.inbound.NettyInboundClient;
...
class InboundClientFactory {
    ...
    synchronized InboundClient newInboundClient(OutboundClientOption option) {
        if (inboundClient == null) {
            inboundClient = new NettyInboundClient(option == null ? new OutboundClientOption() : option);
